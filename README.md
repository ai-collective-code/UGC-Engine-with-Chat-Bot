...
> Insta-agent uses **Render PostgreSQL** (via `pg`), accessed server-side only.
> Live dashboard updates are done by polling the API routes every 4s (no
> WebSocket/Realtime service required).
